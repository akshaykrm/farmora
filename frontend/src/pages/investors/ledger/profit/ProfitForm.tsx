...
const ProfitForm = ({ onSubmit, defaultValues, apiError, onCancel }: Props) => {
  ...
  const seasons = useGetSeasonNames();
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="grid grid-cols-1 gap-4">
        ...
        <SelectList
          ...
          value={
            values.transaction_type_code
              ? (profitTypes.find(
                  (t) => t.code === values.transaction_type_code,
                )?.id ?? null)
              : null
          }
          onChange={(val) => {
        ...
        <DatePicker
          ...
          value={
            values.transaction_date ? dayjs(values.transaction_date) : null
          }
          format="DD-MM-YYYY"
          onChange={(v) => {
